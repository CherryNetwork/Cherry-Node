...
mod tests {
	...
	fn schedule_schedules_including_just_freed() {
		...
		new_test_ext(genesis_config).execute_with(|| {
			...
			// add a couple more parathread claims - the claim on `b` will go to the 3rd parathread
			// core (4) and the claim on `d` will go back to the 1st parathread core (2). The claim
			// on `e` then will go for core `3`.
...

			{
				...
				// cores 0 and 1 are occupied by parachains. cores 2 and 3 are occupied by
				// parathread claims. core 4 was free.
...

			{
				...
				// Although C was descheduled, the core `4`  was occupied so C goes back on the
				// queue.
