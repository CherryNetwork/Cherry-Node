	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, cherry_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						Call::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: cherry_runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						Call::Balances(BalancesCall::transfer {
							...
							value: 100 * DOLLARS - (cherry_runtime::ExistentialDeposit::get() - 1),
						})
					},
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> cherry_runtime::GenesisConfig {
		crate::genesis::config_endowed(
			false,
			Some(cherry_runtime::wasm_binary_unwrap()),
