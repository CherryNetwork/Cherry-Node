...
pub enum DataCommand {
    /// (data, filename) -> (ipfs_address, cid)
...
pub mod pallet {
	...
	pub(super) type ConnectionQueue<T> = StorageValue<_, Vec<ConnectionCommand>, ValueQuery>;

	#[pallet::storage]
    #[pallet::getter(fn data_queue)]
	// A queue of data to publish or obtain on IPFS.
	pub(super) type DataQueue<T> = StorageValue<_, Vec<DataCommand>, ValueQuery>;

	#[pallet::storage]
    #[pallet::getter(fn dht_queue)]
	// A list of requests to the DHT.
	pub(super) type DhtQueue<T> = StorageValue<_, Vec<DhtCommand>, ValueQuery>;
	...
	impl<T: Config> ValidateUnsigned for Pallet<T> {
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			if let Call::submit_ipfs_results{cid, name} = call {
				Self::validate_transaction_parameters(cid.to_vec(), name.to_vec())
			} else {
	...
	impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(0)]
        pub fn ipfs_add_bytes(origin: OriginFor<T>, data: Vec<u8>, cid: Vec<u8>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            <DataQueue<T>>::mutate(|queue| queue.push(DataCommand::AddBytes(data, cid)));
