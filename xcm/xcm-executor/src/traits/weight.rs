...
pub trait WeightTrader: Sized {
	...
	/// Purchase execution weight credit in return for up to a given `fee`. If less of the fee is
	/// required then the surplus is returned. If the `fee` cannot be used to pay for the `weight`,
